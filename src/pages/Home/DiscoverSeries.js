  ...
  const requests = async (page) => {
    ...
    if(page) {
      ...
      const req = await fetch(
        Constants.URL.DISCOVER_TV_URL + Constants.URL.API_KEY + Constants.URL.LANGUAGE + sortBy + Constants.URL.PAGE_URL + page + Constants.URL.LIMIT_RELEASE_DATE + date + Constants.URL.WITH_GENRES_URL + selectedGenres.toString()
      );
      console.log(Constants.URL.DISCOVER_TV_URL + Constants.URL.API_KEY + Constants.URL.LANGUAGE + sortBy + Constants.URL.PAGE_URL + page + Constants.URL.LIMIT_RELEASE_DATE + date + Constants.URL.WITH_GENRES_URL + selectedGenres.toString())
        ...
        {!loading && (
          <View style={[styles.content, {display: loaded == true ? 'flex' : 'none'}]}>
            ...
            <ScrollView horizontal={true} justifyContent="flex-start" style={{width: (Dimensions.get("window").width * 362) / 392.72, marginBottom: 20}}>
                <TouchableOpacity style={[styles.chipArea, {flexDirection: 'row', alignItems: 'flex-end', backgroundColor: '#FFF'}]} onPress={()=>navigation.navigate("SearchFilter", {genres: genres, mediaType: mediaType, sortBy: sortBy})}>
              ...
              if(media.poster_path != null) {
                return (
                  <TouchableOpacity onPress={()=>navigation.navigate("Serie", {mediaId: media.id})} key={index} style={styles.mediaItem}>
                    ...
                    />
                    <Text style={styles.mediaTitle}>{media.name}</Text>
                  </TouchableOpacity>
                );
              }
            })}
