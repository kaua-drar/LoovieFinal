...
export default function MovieEmCartaz({ navigation, route }) {
  ...
  const tryDirector = () => {
    try {
      return <Text style={styles.mediaDetail}>Diretor: {movie.director}</Text>;
    } catch {
      return null;
    }
  };
  ...
  const tryCast = () => {
    try {
      return <Text style={styles.mediaDetail}>Elenco: {movie.cast}</Text>;
    } catch {
      return null;
    }
  };
        ...
        <View style={styles.mediaDetail}>
          <View style={{ flexDirection: "row", alignItems: "flex-end" }}>
            <Text style={styles.note}>
              {(movie.rottenTomatoe.criticsScore / 10).toFixed(1)}
            </Text>
            <Text style={styles.noteof}>/10</Text>
          </View>
          <Star
            score={(movie.rottenTomatoe.criticsScore * 5) / 100}
  ...
  const tryDuration = () => {
    try {
      return (
        <Text style={styles.mediaDetail}>
          Duração: {Math.floor(movie.duration / 60)}h {movie.duration % 60}m
        </Text>
      );
    } catch {
      return null;
    ...
    >
      <View style={styles.headerMenu}>
        <TouchableOpacity
          style={[
            styles.menuButton,
            { backgroundColor: menu == true ? "#292929" : "transparent" },
          ]}
          onPress={() => setMenu(true)}
          disabled={menu == true ? true : false}
        >
        ...
        <TouchableOpacity
          style={[
            ...
            {
              marginLeft: 10,
              backgroundColor: menu == false ? "#292929" : "transparent",
            },
          ]}
          onPress={() => setMenu(false)}
          disabled={menu == false ? true : false}
        >
      ...
      {!loading && !menu && (
        <View style={styles.sessions}>
          ...
          <View
            style={{
              maxHeight: 75,
              maxWidth: (Dimensions.get("window").width * 372) / 392.72,
              marginTop: 25,
            }}
          >
            <ScrollView
              ...
              {sessions.map((item, index) => {
                if (item.isToday == true) {
                  ...
                  return (
                    ...
                    >
                      <DateText>
                        {item.isToday == true ? "hoje" : item.dateFormatted}
                      </DateText>
                  ...
                  return (
                    <View key={index}>
                      ...
                      {v.rooms.map((x, index) => {
                        return (
                          <TouchableOpacity
                            style={styles.sessionArea}
                            key={index}
                            onPress={() =>
                              openBrowserAsync(`${x.sessions[0].siteURL}`)
                            }
                          >
                            <View style={styles.sessionInfo}>
                              <Text
                                style={[styles.sessionText, { minWidth: 65 }]}
                              >
                                {x.name}
                              </Text>
